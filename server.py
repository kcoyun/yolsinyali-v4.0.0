...
# ─── Territory (Bölge) Socket Olayları ───────────────────────────────────────

# Bellekte bölgeler: { id: {id, name, username, color, path, area} }
territory_registry: dict = {}

@socketio.on("territory_created")
def on_territory_created(data):
    if not data or not data.get("id"):
        return
    territory_registry[data["id"]] = data
    emit("territory_created", data, broadcast=True, include_self=False)
    print(f"[TERRITORY] {data.get('username')} → '{data.get('name')}' oluşturdu")

@socketio.on("territory_deleted")
def on_territory_deleted(data):
    if not data or not data.get("id"):
        return
    territory_registry.pop(data["id"], None)
    emit("territory_deleted", data, broadcast=True, include_self=False)
    print(f"[TERRITORY] {data.get('id')} silindi")

@socketio.on("territory_captured")
def on_territory_captured(data):
    if not data or not data.get("id"):
        return
    tid = data["id"]
    if tid in territory_registry:
        territory_registry[tid]["username"] = data.get("capturedBy")
        territory_registry[tid]["color"] = data.get("newColor", "#fff")
    emit("territory_captured", data, broadcast=True, include_self=False)
    print(f"[TERRITORY] {data.get('capturedBy')} → {data.get('previousOwner')} bölgesini ele geçirdi")

# join olayını override ederek mevcut bölgeleri de gönder
@socketio.on("join_with_territories")
def on_join_with_territories(data):
    username = (data.get("username") or "").strip() if data else ""
    if not username:
        return
    if username in active_users_registry:
        active_users_registry[username]["sid"] = request.sid
    join_room(username)
    # Kullanıcı snapshot + bölge snapshot
    emit("active_users_snapshot", {
        "users": [
            {"username": u, "latitude": d["latitude"], "longitude": d["longitude"]}
            for u, d in active_users_registry.items() if u != username
    ...
    emit("territories_snapshot", {
        "territories": list(territory_registry.values())
    })
