...
def update_location():
    ...
    print(f"[LOC] {username}: {latitude:.5f}, {longitude:.5f}", flush=True)
...
def stop_location():
    ...
    if removed:
        ...
        print(f"[STOP] {username} kaldırıldı.", flush=True)
...
@socketio.on("connect")
def on_connect():
    print(f"[WS] Bağlandı: {request.sid}", flush=True)
...
def on_add_pin(data):
    ...
    print(f"[PIN] {username} → {category} @ {latitude:.5f},{longitude:.5f} ({duration_hours}sa)", flush=True)
...
def on_remove_pin(data):
    ...
    print(f"[PIN] {username} kaldırdı: {pin_id}", flush=True)
...
def pin_expiry_worker():
    ...
    while True:
        ...
        try:
            ...
            print(f"[PIN-CLEANUP] hata: {e}", flush=True)
...
def keep_alive_worker():
    ...
    if not render_url:
        return
    print(f"[KEEP-ALIVE] Aktif → {render_url}/ping", flush=True)
    while True:
        time.sleep(600)
        try:
            with urllib.request.urlopen(f"{render_url}/ping", timeout=10) as r:
                print(f"[KEEP-ALIVE] ping OK ({r.status})", flush=True)
        except Exception as e:
            print(f"[KEEP-ALIVE] ping HATA: {e}", flush=True)
